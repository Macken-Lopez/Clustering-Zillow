...
def granulartwocombocomparison(df, interestingcols, target, n=100):
    ...
    for i in range(0, len(interestingAFkeys)):
        interestingAFtwoComboDict.update(
            {
                interestingAFkeys[i]: list(
                    combinations(
                        list(interestingAF.get(interestingAFkeys[i]).keys()), 2
                    )
                )
            }
        )
    ...
    for i in interestingAFkeys:
        ...
        for j in combolist:
            ...
            data = pd.concat([a.assign(frame=f"{j[0]}"), b.assign(frame=f"{j[1]}")])
...
def scale_and_concat(df, scaler, scaled_vars):
    ...
    scaled_df = pd.DataFrame(
        scaled_array, columns=scaled_column_names, index=df.index.values
    )
...
def partitionslist_with_scaled(
...
def find_k(X_train, cluster_vars, k_range):
    ...
    pct_delta = [
        round(((sse[i] - sse[i + 1]) / sse[i]) * 100, 1) for i in range(len(sse) - 1)
    ]

    # create a dataframe with all of our metrics to compare them across values of k: SSE, delta, pct_delta
    k_comparisons_df = pd.DataFrame(
        dict(k=k_range[0:-1], sse=sse[0:-1], delta=delta, pct_delta=pct_delta)
    )
...
def get_centroids(kmeans, cluster_vars, cluster_name):
    ...
    centroid_df = (
        pd.DataFrame(kmeans.cluster_centers_, columns=centroid_col_names)
        .reset_index()
        .rename(columns={"index": cluster_name})
    )
...
def assign_clusters(kmeans, cluster_vars, cluster_name, centroid_df, X):
    for i in range(len(X)):
        clusters = pd.DataFrame(
            kmeans.predict(X[i][cluster_vars]), columns=[cluster_name], index=X[i].index
        )

        clusters_centroids = clusters.merge(
            centroid_df, on=cluster_name, copy=False
        ).set_index(clusters.index.values)
...
def pearsonsRsquareLogError(df, a):
    ...
    if p > alpha:
        corstring = (
            f"No, we observe that {i[0]} and {i[1]} show no correlation by pearsons R:"
        )
        display(nullsym, null, rejnull)
...
def threeQandA_stats_viz_counties(train_la, orange_train, ventura_train):
    ...
    for i in traincounty2combos:
        ...
        a = mapper.get(i[0])
        b = mapper.get(i[1])
        data = pd.concat([a.assign(frame=f"{i[0]}"), b.assign(frame=f"{i[1]}")])
        ...
        if p / 2 > alpha:
            ...
            equalpopstring = (
                f"No, we observe that {i[0]} and {i[1]} are statistically the same:"
            )
            display(nullsym, null, rejnull)
            ...
            equalpopstring = (
                f"Yes, we observe that {i[0]} and {i[1]} are statistically different"
            )
            display(nullsym, null, rejnull)
            print(f"{equalpopstring}\nHence, we reject our null hypothesis\n\n")
